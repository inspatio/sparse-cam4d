...
def center_crop_pil_image(input_image, target_width=1024, target_height=576):
    ...
    if h_ratio > w_ratio:
        ...
        input_image_np = np.asarray(input_image) # HWC
        input_image_np = input_image_np[h-target_height:, :, :]
        return Image.fromarray(input_image_np), ratio
    ...
    return ImageOps.fit(input_image, (target_width, target_height), Image.BICUBIC), ratio
...
def load_images(folder_or_list, size, square_ok=False,force_1024 = False):
    ...
    for path in folder_content:
        ...
        if force_1024:
            img, ratio = center_crop_pil_image(img)
        img_ori = img
        W1, H1 = img.size
        if size == 224:
        ...
        imgs.append(dict(img=ImgNorm(img)[None], true_shape=np.int32(
            [img.size[::-1]]), idx=len(imgs), instance=str(len(imgs)), img_ori=ImgNorm(img_ori)[None], resize_ratio = ratio * (W1/size)))
