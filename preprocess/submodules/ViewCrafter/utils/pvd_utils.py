...
def save_colmap_cameras(ori_size, intrinsics, camera_file):
    with open(camera_file, 'w') as f:
        ...
        # for i, (focal, center) in enumerate(zip(focals, centers), 1):  # Starting index at 1
        #     width, height = ori_size
        #     if i == 1:
        #         print(center)
        #     scale_factor_x = width/2  / center[0]
        #     scale_factor_y = height/2  / center[1]
            ...
            # f.write(f"{i} PINHOLE {width} {height} {focal*scale_factor_x} {focal*scale_factor_x} {width/2} {height/2}\n")               # scale focal
...
def save_colmap_images(poses, images_file):
    with open(images_file, 'w') as f:
        for i, pose in enumerate(poses, 1):  # Starting index at 1
            ...
            f.write(f"{i} {q[0]} {q[1]} {q[2]} {q[3]} {t[0]} {t[1]} {t[2]} {1} frame_{i:05d}.png\n")
            f.write(f"\n")
def save_colmap_points3D(points3D, points3D_file):
    if len(points3D) == 0:
        mean_track_length = 0
    else:
    ...
    with open(points3D_file, "w") as fid:
        fid.write(HEADER)
...
def storePly(path, xyz, rgb):
    ...
    ply_data.write(path)
def save_colmap(H, W, intrinsics, c2ws, pts3d, imgs, confidence_masks, output_path):
    # focals = focals.detach().cpu().numpy()[::-1]
    # centers = centers.detach().cpu().numpy()[::-1]
    intrinsics = intrinsics.detach().cpu().numpy()
    c2ws = c2ws.detach().cpu().numpy()
    output_colmap_path = os.path.join(output_path, "sparse/0")
    os.makedirs(output_colmap_path, exist_ok=True)
    output_colmap_camera = os.path.join(output_colmap_path, "cameras.txt")
    output_colmap_images = os.path.join(output_colmap_path, "images.txt")
    save_colmap_cameras((W, H), intrinsics, output_colmap_camera)
    save_colmap_images(c2ws, output_colmap_images)
    ...
    points3D = {}
    for point3D_id in range(pts_4_3dgs.shape[0]):
        points3D[point3D_id] = Point3D(
                    id=point3D_id,
                    xyz=pts_4_3dgs[point3D_id],
                    rgb=color_4_3dgs[point3D_id],
                    error=0,
                    image_ids=[0],
                    point2D_idxs=[0],
                )
    output_colmap_images = os.path.join(output_colmap_path, "points3D.txt")
    save_colmap_points3D(points3D, output_colmap_images)
    pts_4_3dgs_all = np.array(pts3d).reshape(-1, 3)
    np.save(output_colmap_path + "/pts_4_3dgs_all.npy", pts_4_3dgs_all)

    # np.save(output_colmap_path + "/focal.npy", focals)

def save_depth(data, folder):
    os.makedirs(folder, exist_ok=True)
    if isinstance(data, np.ndarray):
        numpy_data = data.astype(np.float32)
    elif isinstance(data, torch.Tensor):
        numpy_data = data.detach().cpu().numpy().astype(np.float32)
    elif isinstance(data, list):
        numpy_data = np.asarray([d.detach().cpu().numpy().astype(np.float32) for d in data])

    numpy_data = numpy_data.squeeze()
    max_depth = np.max(numpy_data.flatten())
    min_depth = np.min(numpy_data.flatten())
    for i in range(numpy_data.shape[0]):
        np.save(os.path.join(folder, f"frame_{i:05d}.npy"), numpy_data[i])

        tensor_data = ((torch.from_numpy(numpy_data[i])) - min_depth) / (max_depth - min_depth)
        torchvision.utils.save_image(tensor_data.float(), os.path.join(folder, f"frame_{i:05d}.png"))


def save_render(data, images_path, folder=None):
    os.makedirs(images_path, exist_ok=True)

    if isinstance(data, np.ndarray):
        tensor_data = (torch.from_numpy(data)).float()
    elif isinstance(data, torch.Tensor):
        tensor_data = (data.detach().cpu()).float()
    elif isinstance(data, list):
        ...
        tensor_data = torch.from_numpy(stacked_images).float() / 255.0
    for i in range(tensor_data.shape[0]):
        torchvision.utils.save_image(tensor_data[i].permute((2, 0, 1)), os.path.join(images_path, f"frame_{i:05d}.png"))
...
def generate_traj_interp(c2ws,H,W,fs,c,ns,device,viz_traj=False, save_dir = None):

    c2ws = interp_traj(c2ws,n_inserts= ns,device=device)
    num_views = c2ws.shape[0]
    R, T = c2ws[:,:3, :3], c2ws[:,:3, 3:]
    ## 将dust3r(colmap)坐标系转成pytorch3d坐标系
    R = torch.stack([-R[:,:, 0], -R[:,:, 1], R[:,:, 2]], 2) # from RDF to LUF for Rotation
    new_c2w = torch.cat([R, T], 2)
    w2c = torch.linalg.inv(torch.cat((new_c2w, torch.Tensor([[[0,0,0,1]]]).to(device).repeat(new_c2w.shape[0],1,1)),1))
    R_new, T_new = w2c[:,:3, :3].permute(0,2,1), w2c[:,:3, 3] # convert R to row-major matrix
    ...
    if viz_traj:
        ...
        # frames = []
        # cam_info = {}
        # cam_info["focal_length"] = fs.detach().cpu().squeeze().numpy().tolist()
        # cam_info["principal_point"] = c.detach().cpu().squeeze().numpy().tolist()
        # cam_info["transform_matrix"] = {}
        # for i in range(len(poses)):
        #     idx = num_views - 1 - i
        #     cam_info["transform_matrix"][i] = poses[idx].tolist()
        #     frames.append(visualizer_frame(poses, idx))
        # save_video(np.array(frames)/255.,os.path.join(save_dir,'viz_traj.mp4'))
        # with open(os.path.join(save_dir, 'cams.json'), 'w') as f:
        #     json.dump(cam_info, f, indent=True)
...
# ── VGGT additions ────────────────────────────────────────────────────────────
# Functions below are merged from VGGT/pvd_utils.py (not present in ViewCrafter upstream).
# generate_traj_interp and interp_traj replace the ViewCrafter versions because
# vggt_utils.py calls generate_traj_interp with a `mode` keyword argument.
...
def storePlywithTime(path, xyz, rgb, time=None):
    if time is None:
        dtype = [('x', 'f4'), ('y', 'f4'), ('z', 'f4'),
                 ('nx', 'f4'), ('ny', 'f4'), ('nz', 'f4'),
                 ('red', 'u1'), ('green', 'u1'), ('blue', 'u1')]
...
def loadPly(path):
    ...
    timestamps = vertices['time'][:, None] if 'time' in vertices else None
...
def render_pcd(pts3d, imgs, masks, views, renderer, device, nbv=False):
    ...
    if masks is None:
        pts = torch.from_numpy(np.concatenate([p for p in pts3d])).view(-1, 3).to(device).float()
        col = torch.from_numpy(np.concatenate([p for p in imgs])).view(-1, 3).to(device).float()
    else:
        masks = to_numpy(masks)
        if len(pts3d.shape) == 3:
            pts = torch.from_numpy(np.concatenate([p[m.ravel()] for p, m in zip(pts3d, masks)])).to(device).reshape(-1, 3).float()
        else:
            pts = torch.from_numpy(np.concatenate([p[m] for p, m in zip(pts3d, masks)])).to(device).reshape(-1, 3).float()
        col = torch.from_numpy(np.concatenate([p[m] for p, m in zip(imgs, masks)])).to(device).reshape(-1, 3).float()
...
def interp_traj(c2ws: torch.Tensor, n_inserts: Union[int, list] = 25) -> torch.Tensor:
    """VGGT version: supports list n_inserts; no device arg needed (uses c2ws.device)."""
    if n_inserts == 0:
        return c2ws
    n_poses = c2ws.shape[0]
    interpolated_poses = []
    for i in range(n_poses - 1):
        n_inserts_i = n_inserts[i] if isinstance(n_inserts, list) else n_inserts
        start_pose = c2ws[i]
        end_pose = c2ws[(i + 1) % n_poses]
        interpolated_path = interpolate_poses_spline(
            torch.stack([start_pose, end_pose])[:, :3, :].cpu().numpy(), n_inserts_i
        ).to(c2ws.device)
        interpolated_path = interpolated_path[:-1]
        interpolated_poses.append(interpolated_path)
    interpolated_poses.append(c2ws[-1:])
...
def generate_traj_interp(c2ws, H, W, fs, c, ns, mode, viz_traj=False, save_dir=None):
    """VGGT version: accepts `mode` kwarg ('vggt' or 'mast3r')."""
    device = c2ws.device
    c2ws = interp_traj(c2ws, n_inserts=ns)
    num_views = c2ws.shape[0]

    if mode == 'vggt':
        c2ws_pytorch3d = deepcopy(c2ws)
        c2ws_pytorch3d[:, :, 0] *= -1
        c2ws_pytorch3d[:, :, 1] *= -1
        R_row = c2ws_pytorch3d[:, :3, :3]
        w2c = torch.linalg.inv(c2ws_pytorch3d)
        T = w2c[:, :3, -1]
        image_size = ((H, W),)
        if ns > 2:
            fs = interpolate_sequence(fs, ns - 2, device=fs.device)
            c = interpolate_sequence(c, ns - 2, device=c.device)
        cameras = PerspectiveCameras(focal_length=fs, principal_point=c, in_ndc=False,
                                     image_size=image_size, R=R_row, T=T, device=device)
    elif mode == 'mast3r':
        R, T = c2ws[:, :3, :3], c2ws[:, :3, 3:]
        R = torch.stack([-R[:, :, 0], -R[:, :, 1], R[:, :, 2]], 2)
        new_c2w = torch.cat([R, T], 2)
        w2c = torch.linalg.inv(torch.cat((new_c2w,
                                           torch.Tensor([[[0, 0, 0, 1]]]).to(device).repeat(new_c2w.shape[0], 1, 1)), 1))
        R_new, T_new = w2c[:, :3, :3], w2c[:, :3, 3]
        R_new = R_new.permute(0, 2, 1)
        image_size = ((H, W),)
        fs = torch.ones((num_views, 1)).to(fs.device) * fs[0]
        c = c[0].unsqueeze(0).repeat(num_views, 1)
        cameras = PerspectiveCameras(focal_length=fs, principal_point=c, in_ndc=False,
                                     image_size=image_size, R=R_new, T=T_new, device=device)

    if save_dir:
        poses = c2ws.cpu().numpy()
        visualizer(poses, os.path.join(save_dir, 'viz_traj.png'))
        cam_info = {}
        fl = fs[0].detach().cpu().squeeze().numpy().tolist()
        pp = c[0].detach().cpu().squeeze().numpy().tolist()
        cam_info.update({"fl_x": fl, "fl_y": fl, "cx": pp[0], "cy": pp[1], "w": W, "h": H,
                         "transform_matrix": {}})
        for i in range(len(poses)):
            cam_info["transform_matrix"][f"frame_{i:05d}.png"] = poses[i].tolist()
        with open(os.path.join(save_dir, 'cameras_interp.json'), 'w') as f:
