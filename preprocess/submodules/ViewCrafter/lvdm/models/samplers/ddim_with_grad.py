...
class DDIMSamplerWithGrad(object):
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      operated_image=None,operation=None,
                      **kwargs):
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.p_sample_ddim(img, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                                      ...
                                      operated_image=operated_image,operation=operation,
                                      **kwargs)
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      ...
                      operated_image=None,operation=None,**kwargs):
        ...
        for j in range(num_steps):
            if operation.guidence_3:
                torch.set_grad_enabled(True)
                x_in = x.detach().requires_grad_(True)

                # #--------------------------- Get e_t -------------------------#
                if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
                    model_output = self.model.apply_model(x_in, t, c, **kwargs) # unet denoiser
                else:
                    ### do_classifier_free_guidance
                    if isinstance(c, torch.Tensor) or isinstance(c, dict):
                        e_t_cond = self.model.apply_model(x_in, t, c, **kwargs)
                        e_t_uncond = self.model.apply_model(x_in, t, unconditional_conditioning, **kwargs)
                    else:
                ...
                if self.model.parameterization == "v":
                    e_t = self.model.predict_eps_from_z_and_v(x_in, t, model_output)
                else:
                    e_t = model_output
                ...
                # #------------------------- current prediction for x_0 (pred_x0) from x_in -----------------------------#
                if self.model.parameterization != "v":
                    pred_x0 = (x_in - sqrt_one_minus_at * e_t) / a_t.sqrt()
                else:
                    pred_x0 = self.model.predict_start_from_z_and_v(x_in, t, model_output)
                if self.model.use_dynamic_rescale:
                ...
                recons_image = self.model.decode_first_stage(pred_x0)
                # #--------------------------- Calculate Gradient -------------------------------------------#
                ...
                if op_im is not None:
                    ...
                    del x_in, pred_x0, recons_image, op_im, selected, grad
                else:
                    e_t = e_t
                    x_in = x_in.requires_grad_(False)
                    del x_in, pred_x0, recons_image, op_im
            ...
            with torch.no_grad():
                # #------------------------- current prediction for x_0 (pred_x0) -----------------------------#
                if self.model.parameterization != "v":
