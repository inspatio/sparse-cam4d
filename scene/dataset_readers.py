...
class CameraInfo(NamedTuple):
    ...
    FovY: np.array
    FovX: np.array
    image: Union[np.array, Image.Image, None]
    depth: Union[np.array, None]
    image_path: str
    image_name: str
    width: int
    height: int
...
def readCamerasFromTransforms2(path, transformsfile, white_background, extension=".png", time_duration=None, frame_ratio=1, dataloader=False,
                               target_cam=[], mode='train', scene_scale_ratio=1.0, camera_downsample=1):
    cam_infos = []
    with open(os.path.join(path, transformsfile)) as json_file:
        contents = json.load(json_file)
    #! Load frames info
    ...
    def frame_read_fn(idx_frame):
        ...
        #! Filter Pseudo Camera Poses
        if mode=='train' and not is_true_image:
            camera_idx = int(frame["file_path"].split('/')[-1].split('.')[0].split('_')[-1]) # time_xxxx/diffusion/frame_xxxxx.png
            if camera_idx % camera_downsample:
                return
        ...
        #! Filter Target Camera
        if len(target_cam) > 0:
            if is_true_image:
                cam = frame["file_path"].split('/')[-1].split('.')[0].split('_')[0] # cam01/cam01_0000.png
            else:
                cam = frame["file_path"].split('/')[-3] # time_xxxx/diffusion/frame_xxxxx.png
            if not cam in target_cam:
                return

        #! Get Camera Pose
        c2w = np.array(frame["transform_matrix"]) # COLMAP coordinates
        # get the world-to-camera transform and set R, T
        w2c = np.linalg.inv(c2w)
        R = np.transpose(w2c[:3,:3])  # R is stored transposed due to 'glm' in CUDA code
        ...
        #! Organize Camera Information
        if 'fl_x' in frame or 'fx' in frame:
            if 'id' in frame:
                camera_id = frame['id']
            else:
                camera_id = int(frame["file_path"].split('/')[-1][3:5]) # images/cam01_0000.png -> 1
            cx = frame['cx']
            cy = frame['cy']
            cx = cx / frame["width"] * width
            cy = cy / frame["height"]* height
            fl_x = frame['fx'] if 'fx' in frame else frame['fl_x']
            fl_y = frame['fy'] if 'fy' in frame else frame['fl_y']
            FovX = focal2fov(fl_x, frame["width"])
            fl_x = fov2focal(FovX, width)
            FovY = focal2fov(fl_y, frame["height"])
            fl_y = fov2focal(FovY, height)
            cam_info = CameraInfo(camera_id=camera_id, uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, depth=depth,
                        ...
                        fl_x=fl_x, fl_y=fl_y, cx=cx, cy=cy, is_true_image=is_true_image)
        else:
            if 'fl_x' in contents:
                fl_x = contents['fl_x']
                fl_y = contents['fl_y']
                ...
                fl_y = fov2focal(FovY, height)
            elif "camera_angle_x" in contents:
                FovX = contents["camera_angle_x"]
                fl_x = fov2focal(FovX, width)
                fl_y = fl_x
                ...
                raise ValueError(f"Camera intrinsic parameters not found in {transformsfile}")
...
def readNerfSyntheticInfo2(path, white_background, eval, extension=".png", num_pts=100_000, time_duration=None, num_extra_pts=0, frame_ratio=1, dataloader=False, target_cam=[],
                           transforms_file='cameras.json', ply_file="mast3r_dense_pts_merged.ply", scene_scale_ratio=1.0, camera_downsample=8):
    if isinstance(target_cam, Dict):
        target_cam_train = target_cam["train"]
        target_cam_test = target_cam["test"]
    else:
        target_cam_train = target_cam
        target_cam_test = ['time_0000']

    logger.info(f"{transforms_file=}")
    ...
    train_cam_infos = readCamerasFromTransforms2(path, transforms_file, white_background, extension,
                                                ...
                                                target_cam=target_cam_train, camera_downsample=camera_downsample)
    assert len(train_cam_infos)
    ...
    test_cam_infos = readCamerasFromTransforms2(path, transforms_file, white_background, extension,
                                                time_duration=time_duration, frame_ratio=frame_ratio, dataloader=dataloader, scene_scale_ratio=scene_scale_ratio,
                                                target_cam=target_cam_test, mode='test', camera_downsample=camera_downsample)

    if not eval:
        train_cam_infos.extend(test_cam_infos)
        test_cam_infos = []

    logger.info(f"{len(train_cam_infos)} Training views with {len(set([cam.timestamp for cam in train_cam_infos]))} timestamps, {len(test_cam_infos)} Test views loaded")
    logger.info(f"{len([c for c in train_cam_infos if c.depth is not None])} Training cameras with depth prior")
    nerf_normalization = getNerfppNorm(train_cam_infos)
